...
public class AdminController {
	...
	@Autowired UserAllergyDao userAllergyDao;
	...
	public String adminMainController(HttpSession session, Model model) {
		...
		for(User User : user){
			if(!User.getPhoto().equals("")){
					 String files = User.getPhoto();
					 JSONObject fileObj = JSONObject.fromObject(files);
					 UserfileList.add(fileObj);
			}else{
				UserfileList.add("");
			}
		}
		model.addAttribute("fileList", fileList);
		model.addAttribute("photo", UserfileList);
	...
	public String userListController(
		...
		model.addAttribute("photo", fileList);
		// 페이징
	...
	public String userViewController(
		...
		UserAllergy ua = userAllergyDao.getUserAllergy(userId);
		List fileList = new ArrayList<>();
		if(!user.getPhoto().equals("")){
			 String files = user.getPhoto();
			 JSONObject fileObj = JSONObject.fromObject(files);
	...
	public String userEditController(
		@RequestParam(value = "userId", required = false, defaultValue = "") String userId,
		HttpSession session, Model model) {
	...
	public String userDiaryController(
		...
		List<Bbs> list = bbsDao.getBbsList(userId,"","","",0,0, page, ITEM_COUNT_PER_PAGE);
		int count = bbsDao.getBbsCount(userId,"","","",0,0);
		User user = userDao.getUser(userId);
		List fileList = new ArrayList<>();
		if(!user.getPhoto().equals("")){
			 String files = user.getPhoto();
		...
		model.addAttribute("fileList", UserfileList);
		model.addAttribute("photo", fileList);
		model.addAttribute("user", user);
		// 페이징
		String paging = Paging.getPaging(page, count, ITEM_COUNT_PER_PAGE, PAGE_COUNT_PER_PAGING);
...
/*	keyword		: 	listForm.keyword.value,
	...
	public String DiaryListController(
			...
			HttpSession session, Model model) {
		//String userId,String keyword,String startDate,String endDate,int gender
		List<Bbs> list = bbsDao.getBbsList("",keyword,startDate,endDate,gender,0,page, ITEM_COUNT_PER_PAGE);
		int count = bbsDao.getBbsCount("",keyword,startDate,endDate,gender,0);
		...
		List UserfileList = new ArrayList<>();
		for(Bbs bbs : list){
			if(!bbs.getPhoto().equals("")){
				 String files = bbs.getPhoto();
				 JSONObject fileObj = JSONObject.fromObject(files);
				 UserfileList.add(fileObj);
			}else{
				UserfileList.add("");
	...
	public String DiaryViewController(
		...
		List<BbsComment> commentList = bbsCommentDao.getBbsCommentAdminList(bbs.getBbsSeq(),page,ITEM_COUNT_PER_PAGE);
		int count = bbsCommentDao.getBbsCommentAdminCount(bbsSeq);
		...
		List UserfileList = new ArrayList<>();
			for(BbsComment bc : commentList){
				if(!bc.getPhoto().equals("")){
					 String files = bc.getPhoto();
					 JSONObject fileObj = JSONObject.fromObject(files);
					 UserfileList.add(fileObj);
				}else{
					UserfileList.add("");
	...
	public String MissionViewController(
		...
		for (MissionComment mc : commentList) {
			...
			if(!mc.getPhoto().equals("")){
				 String files = mc.getPhoto();
				 JSONObject fileObj = JSONObject.fromObject(files);
				 UserfileList.add(fileObj);
			}else{
				UserfileList.add("");
	...
	// 미션 참여중 멤버
	@RequestMapping("/admin/mission/mission_member.go")
	public String MissionMemberController(
		...
		List<User>list = missionUserDao.getMissionUserList(missionSeq, page, ITEM_COUNT_PER_PAGE);
		...
		return "/admin/mission/mission_member";
	}
	...
	public String MissionEditController(
		...
		try{
			...
			if(missionSeq <1){//등록
				...
				int seq = missionDao.getLastSeq();
				result=true;
				List<User>list = userDao.getPushUserList();
				for(User user:list){
					push(user.getUserId(), seq, "New Mission", Push.MSG_TYPE_SEND_MISSION, userId);
				}
	...
	public String noticeEditDoController(HttpServletRequest req,
		...
		try {
			if (noticeSeq == 0) {
				...
				message = "complete.";
				if(sendPush==1){
					list = userDao.getPushUserList();
					for(User user:list){
						push(user.getUserId(), noticeSeq, "New Notice", Push.MSG_TYPE_SEND_NOTICE, userId);
					}
				}
			...
			if(!photo.equals("")){
				for (int i=0; i<photoArr.length; i++) {
					if(photoArr[i]!=""&&thumbArr[i]!=""){
						BufferedImage bi = ImageIO.read(new File(FILE_ROOT + photoArr[i]));
