...
public class NoticeDao {
	...
	public List<Notice> getNoticeMainListsort(int page, int itemCountPerPage,int notiType) {
	...
	public List<Notice> getNoticeMainList(String keyword, int page, int itemCountPerPage,int notiType) {
		String query = ""
				...
				+ itemCountPerPage + ") +1 AND " + page + " * "
				+ itemCountPerPage + " ";
		return (List<Notice>) this.jdbcTemplate.query(query,new Object[] {notiType,"%" + keyword + "%","%" + keyword + "%"}, this.VnoticeMapper);
	}

	public int getNoticeMainCount(String keyword,int notiType) {
		String query = " SELECT COUNT(*) FROM V_NF_NOTICE WHERE (noti_type = ? AND title like ? OR contents_text like ? ) ";
